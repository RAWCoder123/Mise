...
const migration = readFileSync(
  "supabase/migrations/20260828043000_operator_display_name_read.sql",
  "utf8"
);
...
test("Expo loads display names without caller-selected user ids and saves through update_my_profile", () => {
  const hostedLoad =
    supabaseRepository.match(/async fetchMyDisplayName\([\s\S]*?\n    \},/)?.[0] ?? "";
  const hostedSave =
    supabaseRepository.match(/async updateMyProfile\([\s\S]*?\n    \},/)?.[0] ?? "";
  assert.match(hostedLoad, /\.rpc\(\s*["']get_my_display_name["']/);
  assert.doesNotMatch(hostedLoad, /p_user_id|userId/);
  assert.match(hostedSave, /\.rpc\(\s*["']update_my_profile["']/);
  assert.doesNotMatch(hostedSave, /\.from\(\s*["']users["']/);
...
test("settings exposes an operator profile display-name screen without push secrets", () => {
  ...
  assert.match(profileScreen, /normalizeOperatorDisplayName/);
