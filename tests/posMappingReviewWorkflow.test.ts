...
test("mapping review UI presents suggestions as provisional and updates the queue after decisions", () => {
  ...
  assert.match(screen, /visibleQueue\.menuItems\.map/);
  ...
  assert.match(screen, /visibleQueue\.pendingCount === 0/);
  assert.match(screen, /action=\{String\(visibleQueue\.pendingCount\)\}/);
  ...
  assert.match(screen, /hubReady \? queue : null/);
  assert.match(screen, /hasLoadedRef/);
  assert.match(screen, /const soft = hasLoadedRef\.current && activeRestaurantIdRef\.current === restaurantId/);
  assert.match(screen, /Soft refresh must preserve operator menu-item selections/);
  assert.match(screen, /if \(!soft\) \{\s*setQueue\(null\)/);
  assert.match(screen, /setSelectedMenuItemIds\(\{\}\)/);
  assert.match(screen, /setLoadedRestaurantId\(null\)/);
  assert.match(screen, /!actionsEditable/);
