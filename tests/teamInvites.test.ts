...
test("invite tokens are opaque 64-char hex values", async () => {
  ...
  assert.equal(buildInviteClaimDeepLink(token), `mise://invite/${token}`);
...
test("demo schema v14 includes member invite storage", () => {
  const seed = createInitialDemoState("Toast");
  assert.equal(seed.schema_version, 14);
  assert.deepEqual(seed.memberInvites, []);
  assert.equal(seed.memberships[0]?.role, "owner");

  const { memberInvites: _ignored, memberships: _m, schema_version: _version, ...legacy } = seed;
  const repaired = repairDemoState({ ...legacy, schema_version: 13 });
...
test("member invite migration and client wiring are present", () => {
  const migration = readFileSync(
    "supabase/migrations/20260828170000_restaurant_member_invites.sql",
    "utf8"
  );
  ...
  assert.match(supabaseRepository, /rpc\("create_restaurant_member_invite"/i);
  assert.match(supabaseRepository, /rpc\("claim_restaurant_member_invite"/i);
  assert.match(demoRepository, /createDemoMemberInvite/i);
