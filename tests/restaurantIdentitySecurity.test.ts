...
test("restaurant identity settings gate edits to owner/admin and save through Edge-owned profile updates", () => {
  ...
  assert.match(hostedSave, /action:\s*"update_restaurant_profile"/);
  assert.doesNotMatch(hostedSave, /\.rpc\(\s*["']update_restaurant_profile["']/);
