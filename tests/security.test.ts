...
test("recipe baseline unlink removes live mappings through the manager-owned workflow", () => {
  const inventoryWorkflow = readFileSync("services/application/inventory.ts", "utf8");
  const repository = readFileSync("services/repositories/supabaseRepository.ts", "utf8");
  const edge = readFileSync("supabase/functions/operational-workflows/index.ts", "utf8");
  const migration = readFileSync("supabase/migrations/20260826120000_delete_recipe_mapping.sql", "utf8");
...
test("package exposes private-beta backend security gates", () => {
  ...
  assert.equal(packageJson.dependencies.expo, "~56.0.20");
  assert.equal(packageJson.dependencies["expo-router"], "~56.2.19");
  assert.equal(packageJson.dependencies["expo-constants"], "~56.0.22");
  assert.equal(packageJson.dependencies["expo-linking"], "~56.0.17");
