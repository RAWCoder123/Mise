...
test("operational screens reject late requests and render only active-restaurant data", () => {
  ...
  assert.match(screens.gmail, /hubUnavailable\s*\?\s*"unavailable"/);
  assert.match(screens.gmail, /actionLabel=\{t\("common\.retry"\)\}/);
  assert.match(screens.gmail, /presentRestaurantScopedHubActionsEditable/);
