-- Route atomic restaurant setup persistence through a service-owned RPC so
-- authenticated Expo clients must use operational-workflows (Edge firewall
-- reservation + rate limit + audit). Ledger semantics stay identical:
-- opening creates and quantity deltas append inventory_movements with
-- manual_count / save_restaurant_setup. Fixed-semantic setup audit RPC is
-- also service-owned; setup itself already records fingerprinted audit rows.

create or replace function private.service_save_restaurant_setup(
...
revoke all on function private.service_save_restaurant_setup(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, integer)
  from public, anon, authenticated, service_role;
grant execute on function private.service_save_restaurant_setup(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, integer)
  to service_role;
...
begin
  ...
  return private.service_save_restaurant_setup(
    auth.uid(),
    p_restaurant_id,
    p_inventory_items,
    p_suppliers,
...
comment on function public.save_restaurant_setup(uuid, jsonb, jsonb, jsonb, jsonb, jsonb, integer) is
  'Legacy auth.uid()-bound setup upsert. Authenticated execute is revoked; use service_save_restaurant_setup through operational-workflows.';

create or replace function public.service_save_restaurant_setup(
  p_actor_user_id uuid,
  p_restaurant_id uuid,
  p_inventory_items jsonb default '[]'::jsonb,
  p_suppliers jsonb default '[]'::jsonb,
...
set search_path = ''
as $$
  select private.service_save_restaurant_setup(
    p_actor_user_id,
    p_restaurant_id,
    p_inventory_items,
    p_suppliers,
...
revoke all on function public.service_save_restaurant_setup(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, integer)
  from public, anon, authenticated, service_role;
grant execute on function public.service_save_restaurant_setup(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, integer)
  to service_role;

comment on function public.service_save_restaurant_setup(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, integer) is
  'Service-owned atomic setup upsert. Authenticated clients must call through operational-workflows. Inventory quantity creates and deltas append inventory_movements (manual_count / save_restaurant_setup).';

create or replace function private.service_record_setup_completion_audit(
  p_actor_user_id uuid,
  p_restaurant_id uuid,
...
revoke all on function private.service_record_setup_completion_audit(uuid, uuid, jsonb)
  from public, anon, authenticated, service_role;
grant execute on function private.service_record_setup_completion_audit(uuid, uuid, jsonb)
  to service_role;
...
begin
  ...
  return private.service_record_setup_completion_audit(auth.uid(), p_restaurant_id, p_metadata);
end;
$$;

revoke all on function public.record_setup_completion_audit(uuid, jsonb)
  from public, anon, authenticated, service_role;

comment on function public.record_setup_completion_audit(uuid, jsonb) is
  'Legacy auth.uid()-bound setup audit helper. Authenticated execute is revoked; setup audit is written by service_save_restaurant_setup.';

create or replace function public.service_record_setup_completion_audit(
  p_actor_user_id uuid,
  p_restaurant_id uuid,
  p_metadata jsonb default '{}'::jsonb
)
...
set search_path = ''
as $$
  select private.service_record_setup_completion_audit(
    p_actor_user_id,
    p_restaurant_id,
    p_metadata
  );
...
revoke all on function public.service_record_setup_completion_audit(uuid, uuid, jsonb)
  from public, anon, authenticated, service_role;
grant execute on function public.service_record_setup_completion_audit(uuid, uuid, jsonb)
  to service_role;

comment on function public.service_record_setup_completion_audit(uuid, uuid, jsonb) is
  'Service-owned fixed-semantic setup audit helper. Prefer service_save_restaurant_setup which already records fingerprinted setup_completed events.';
