-- Allow optional operator notes on manager_correction inventory ledger rows.
-- Notes travel in the inventory patch payload as `note` and are never written to inventory_items.
...
declare
  ...
  safe_note text := nullif(btrim(coalesce(safe_patch->>'note', '')), '');
...
begin
  ...
  if safe_note is not null and char_length(safe_note) > 240 then
    raise exception 'Correction note is outside supported limits' using errcode = '22023';
  end if;

  safe_patch := safe_patch - 'note';
...
comment on function private.service_update_inventory_and_signals(uuid, uuid, uuid, bigint, jsonb, jsonb, jsonb) is
  'Service-owned inventory item patch. Quantity deltas are ledgered as manager_correction with optional note metadata; count sessions keep manual_count.';
