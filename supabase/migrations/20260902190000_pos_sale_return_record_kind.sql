-- Persist Square itemized returns as positive quantity_sold rows with
-- record_kind = 'return'. quantity_sold stays > 0; depletion/planning use a
-- signed delta in application code and in fetch_planning_sales aggregation.

alter table public.pos_sales
  add column if not exists record_kind text not null default 'sale';

alter table public.pos_sales
  drop constraint if exists pos_sales_record_kind_check;

alter table public.pos_sales
  add constraint pos_sales_record_kind_check
  check (record_kind in ('sale', 'return'));

comment on column public.pos_sales.record_kind is
  'sale depletes inventory demand; return reverses it. quantity_sold remains positive.';
...
begin
  ...
  loop
    ...
    sale_record_kind := case
      when coalesce(sale->>'record_kind', 'sale') = 'return' then 'return'
      else 'sale'
    end;
...
grant execute on function private.service_apply_square_sync_result_mise_003a_base(
  uuid, uuid, uuid, jsonb, jsonb, text, date, date
) to service_role;

-- Keep returns as distinct rows (signed math happens in consumers). Sale-only
-- aggregates must not sum return quantities as extra demand.
...
begin
  ...
  provider_complete as (
    select
      ...
      'sale'::text as record_kind
    from windowed_sales sale
    where coalesce(sale.record_kind, 'sale') = 'sale'
      and (
  ...
  provider_incomplete as (
    select
      ...
      sale.provider_location_id,
      sale.provider_variation_id,
      coalesce(sale.record_kind, 'sale') as record_kind
    from windowed_sales sale
    where coalesce(sale.record_kind, 'sale') = 'sale'
      and (
      lower(coalesce(sale.source_pos, '')) in ('square', 'toast', 'clover', 'lightspeed')
  ...
  return_rows as (
    select
      ...
      sale.provider_location_id,
      sale.provider_variation_id,
      'return'::text as record_kind
    from windowed_sales sale
    where coalesce(sale.record_kind, 'sale') = 'return'
  )
