-- Allow managers to correct inventory purchase-unit labels after setup.
-- On-hand quantity remains append-only; changing unit re-runs the existing
-- before-update canonical normalization trigger (verified for standard units,
-- draft for pack/unknown units). supplier_name stays durable-ID only.
...
begin
  ...
  if pg_catalog.jsonb_typeof(safe_patch) <> 'object'
    or safe_patch = '{}'::jsonb
    or safe_patch - array['par_level', 'reorder_threshold', 'unit'] <> '{}'::jsonb
  then
  ...
  if safe_patch ? 'unit' then
    next_unit := pg_catalog.btrim(coalesce(safe_patch->>'unit', ''));
    next_unit := pg_catalog.regexp_replace(next_unit, '\s+', ' ', 'g');
    if pg_catalog.char_length(next_unit) not between 1 and 40 then
      raise exception 'Inventory patch is outside supported limits' using errcode = '22023';
    end if;
    item_row.unit := next_unit;
  end if;
...
comment on function private.service_update_inventory_and_signals(
  ...
  'Service-owned inventory policy patch (par/reorder/purchase unit) with signal refresh. On-hand quantity changes must use record_inventory_event.';
