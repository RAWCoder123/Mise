...
begin
  ...
  insert into public.audit_logs (
    ...
    p_restaurant_id,
    p_actor_user_id,
    case when safe_status = 'fresh' then 'pos_planning_sync_fresh' else 'pos_planning_sync_stale' end,
    'pos_integrations',
    p_integration_id,
    jsonb_build_object(
      'status', safe_status,
...
as $$
  select private.service_record_pos_planning_sync_state(
    p_actor_user_id, p_restaurant_id, p_integration_id, p_status, p_error_code
  );
$$;

revoke all on function private.service_record_pos_planning_sync_state(uuid, uuid, uuid, text, text)
  from public, anon, authenticated, service_role;
revoke all on function public.service_record_pos_planning_sync_state(uuid, uuid, uuid, text, text)
  from public, anon, authenticated, service_role;

grant execute on function private.service_record_pos_planning_sync_state(uuid, uuid, uuid, text, text)
  to service_role;
grant execute on function public.service_record_pos_planning_sync_state(uuid, uuid, uuid, text, text)
  to service_role;
