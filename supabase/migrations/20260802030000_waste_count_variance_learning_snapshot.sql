-- Include waste and manual-count variance samples in the operational planning
-- snapshot so Edge signal refresh can learn bounded chronic waste / count-shrink
-- bias and emit dedicated insights without new business tables.

create index if not exists inventory_movements_restaurant_waste_created_at_idx
  on public.inventory_movements (restaurant_id, created_at desc)
  where reason = 'waste';

create index if not exists inventory_movements_restaurant_manual_count_created_at_idx
  on public.inventory_movements (restaurant_id, created_at desc)
  where reason = 'manual_count';
...
comment on function private.fetch_operational_planning_snapshot(uuid, uuid) is
  'Service-owned planning snapshot for owner/admin/manager/staff, including receivingHistory, wasteHistory, and countVarianceHistory for bounded operational learning. Edge enforces which mutations staff may run.';
