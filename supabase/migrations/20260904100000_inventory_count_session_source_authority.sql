-- Physical counts are authoritative on-hand replaces. They may only enter the
-- append-only ledger from audited count-session approval
-- (`source = 'approve_count_session'` via `approve_count_session`). The public
-- `record_inventory_event` RPC rejects count rows entirely so operators cannot
-- spoof session provenance.

create or replace function private.enforce_inventory_count_session_source()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if new.event_type = 'count'
    and new.source is distinct from 'approve_count_session'
  then
    raise exception 'Inventory counts must come from an approved count session'
      using errcode = '22023';
  end if;
  return new;
end;
...
revoke all on function private.enforce_inventory_count_session_source()
  from public, anon, authenticated, service_role;

drop trigger if exists inventory_events_count_session_source on public.inventory_events;

create trigger inventory_events_count_session_source
  before insert on public.inventory_events
  for each row
  execute function private.enforce_inventory_count_session_source();

comment on function private.enforce_inventory_count_session_source() is
  'Fail closed: inventory_events count rows must stamp source approve_count_session from audited session approval.';
...
begin
  ...
  -- Counts replace on-hand authority and must come from approve_count_session
  -- direct inserts, never this operator RPC (even with a spoofed source).
  ...
  if p_quantity is null
    or (p_event_type in ('receipt', 'waste', 'usage') and p_quantity < 0)
...
comment on function public.record_inventory_event(
  ...
  'Append-only inventory ledger RPC for non-count event types. Physical counts must use approve_count_session.';
