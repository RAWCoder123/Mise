...
create or replace function private.service_update_my_notification_preferences(
...
revoke all on function private.service_update_my_notification_preferences(uuid, jsonb)
  from public, anon, authenticated;
grant execute on function private.service_update_my_notification_preferences(uuid, jsonb)
  to service_role;

create or replace function public.service_update_my_notification_preferences(
  p_actor_user_id uuid,
  p_preferences jsonb
)
returns jsonb
...
set search_path = ''
as $$
  select private.service_update_my_notification_preferences(p_actor_user_id, p_preferences);
$$;

revoke all on function public.service_update_my_notification_preferences(uuid, jsonb)
  from public, anon, authenticated, service_role;
grant execute on function public.service_update_my_notification_preferences(uuid, jsonb)
  to service_role;

comment on function public.service_update_my_notification_preferences(uuid, jsonb) is
  'Service-owned operator notification preference update. Authenticated clients must call through operational-workflows with an active restaurant membership for Edge reservation; the preference row is always the Edge-authenticated actor.';

-- Legacy auth.uid() wrapper kept for SQL compatibility but revoked from clients
-- after Edge ownership (same pattern as preferred_locale).
...
begin
  ...
  return private.service_update_my_notification_preferences(auth.uid(), p_preferences);
end;
$$;
