-- Reject manual POS CSV re-imports that would change sales already applied to inventory.
-- Identical re-imports remain idempotent; quantity/item/date corrections must use count/waste/adjustment.
...
comment on function private.service_ingest_manual_pos_sales(uuid, uuid, jsonb, text) is
  'Service-only bounded manual CSV POS sales ingest; rejects corrections to rows already consumed into inventory.';
