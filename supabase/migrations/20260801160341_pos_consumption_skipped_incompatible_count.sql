-- Surface unit-incompatible recipe skips separately from truly unmapped POS sales.
-- Aligns hosted apply/ingest summaries with the demo planner (skippedIncompatibleCount).
...
declare
  ...
  skipped_incompatible_count integer := 0;
...
begin
  ...
  loop
    ...
    loop
      ...
      for update;
      if not found then
        skipped_incompatible_count := skipped_incompatible_count + 1;
        continue;
      end if;
  ...
  return jsonb_build_object(
    ...
    'skipped_incompatible_count', skipped_incompatible_count
  );
end;
$$;
...
declare
  ...
  payload record;
  existing_sale public.pos_sales%rowtype;
  has_existing_sale boolean := false;
...
begin
  ...
  loop
    ...
    from public.pos_sales sale
    where sale.restaurant_id = p_restaurant_id
      and sale.source_pos = 'Manual CSV Upload'
      and sale.source_record_id = payload.source_record_id
    limit 1;
    has_existing_sale := found;

    has_consumption := exists (
      select 1
      from public.inventory_movements movement
      where movement.restaurant_id = p_restaurant_id
        and movement.reason in ('recipe_consumption', 'pos_consumption')
        and movement.metadata->>'source_record_id' = payload.source_record_id
    );

    if has_consumption and not has_existing_sale then
      raise exception
        'POS CSV row % already drove inventory consumption; correct via count/waste instead of re-import',
        payload.source_record_id
        using errcode = '22023';
    end if;

    if has_consumption
       and has_existing_sale
       and (
         round(existing_sale.quantity_sold::numeric, 4) is distinct from round(payload.quantity_sold::numeric, 4)
         or lower(regexp_replace(trim(existing_sale.item_name), '\s+', ' ', 'g'))
            is distinct from lower(regexp_replace(trim(payload.item_name), '\s+', ' ', 'g'))
         or lower(regexp_replace(trim(existing_sale.category), '\s+', ' ', 'g'))
            is distinct from lower(regexp_replace(trim(payload.category), '\s+', ' ', 'g'))
         or existing_sale.sale_date is distinct from payload.sale_date
       )
    then
      raise exception
        'POS CSV row % already drove inventory consumption; correct via count/waste instead of re-import',
    ...
    -- CSV source fingerprints include quantity, so corrected rows mint new IDs.
    -- Reject same-date/item/category rows that already consumed inventory under another ID.
    if exists (
      select 1
      from public.pos_sales sale
      where sale.restaurant_id = p_restaurant_id
        and sale.source_pos = 'Manual CSV Upload'
        and sale.source_record_id is distinct from payload.source_record_id
        and sale.sale_date = payload.sale_date
        and lower(regexp_replace(trim(sale.item_name), '\s+', ' ', 'g'))
          = lower(regexp_replace(trim(payload.item_name), '\s+', ' ', 'g'))
        and lower(regexp_replace(trim(sale.category), '\s+', ' ', 'g'))
          = lower(regexp_replace(trim(payload.category), '\s+', ' ', 'g'))
        and exists (
          select 1
          from public.inventory_movements movement
          where movement.restaurant_id = p_restaurant_id
            and movement.reason in ('recipe_consumption', 'pos_consumption')
            and movement.metadata->>'source_record_id' = sale.source_record_id
        )
    ) then
      raise exception
        'POS CSV row % already drove inventory consumption; correct via count/waste instead of re-import',
  ...
  insert into public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'skipped_incompatible_count', coalesce((consumption->>'skipped_incompatible_count')::integer, 0)
    )
  );
...
comment on function private.apply_recipe_consumption_for_sales(uuid, uuid, text[]) is
  'Service-only mapped recipe consumption for POS sales; reports unmapped and unit-incompatible skips.';
comment on function private.service_ingest_manual_pos_sales(uuid, uuid, jsonb, text) is
  'Service-only bounded manual CSV POS sales ingest; returns skipped_incompatible_count with consumption summary.';
