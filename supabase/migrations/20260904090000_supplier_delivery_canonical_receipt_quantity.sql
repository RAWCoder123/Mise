-- Convert supplier-delivery purchase-unit line quantities into canonical ledger
-- receipt quantities before calling record_inventory_event.
--
-- Delivery lines (and supplier_delivery_items) remain purchase-unit evidence that
-- matches recommendations / inventory_items.unit. Inventory events must store
-- canonical quantities so projection can divide by canonical_quantity_per_unit.
-- Without this conversion, a 24 lb receive labeled as unit g projects as
-- 24/453.592 ≈ 0.05 lb of on-hand.
--
-- Replaces only the MISE-003B name-keyed base used by the MISE-003C wrapper.
-- Does not rewrite historical under-converted receipts.
...
declare
  ...
  target_item_row public.inventory_items;
  purchase_net_quantity numeric;
  ledger_receipt_quantity numeric;
  unit_price numeric;
...
begin
  ...
  loop
    ...
    -- Delivery line quantities are purchase-unit. Convert to canonical before
    -- writing the inventory ledger so projection divides by the verified factor.
    if substitution_item_id is not null then
      select * into target_item_row
      from public.inventory_items
      where restaurant_id = p_restaurant_id and id = substitution_item_id;
    else
      target_item_row := item_row;
    end if;
    if target_item_row.canonical_quantity_per_unit is null
      or target_item_row.canonical_quantity_per_unit <= 0
      or target_item_row.canonical_quantity_per_unit::text in ('NaN', 'Infinity', '-Infinity')
    then
      raise exception 'Delivery line canonical conversion is not verified' using errcode = '22023';
    end if;

    purchase_net_quantity := received_quantity - damaged_quantity;
    if purchase_net_quantity > 0 then
      ledger_receipt_quantity :=
        purchase_net_quantity * target_item_row.canonical_quantity_per_unit;
      if ledger_receipt_quantity is null
        or ledger_receipt_quantity <= 0
        or ledger_receipt_quantity > 1000000000000
        or ledger_receipt_quantity::text in ('NaN', 'Infinity', '-Infinity')
      then
        raise exception 'Delivery receipt quantity is outside supported limits'
          using errcode = '22023';
      end if;
      select * into event_row
      from public.record_inventory_event(
        p_restaurant_id, coalesce(substitution_item_id, inventory_item_id), 'receipt',
        ledger_receipt_quantity, line_canonical_unit,
        ...
        jsonb_build_object(
          ...
          'purchaseUnitQuantity', purchase_net_quantity,
          'canonicalQuantityPerUnit', target_item_row.canonical_quantity_per_unit
        )
      );
    else
      event_row := null;
...
comment on function public.record_supplier_delivery_mise_003b_name_base(
  ...
  'MISE-003B supplier delivery base. Purchase-unit line quantities are converted to canonical ledger quantities using the receive target item''s verified canonical_quantity_per_unit before inventory projection.';
