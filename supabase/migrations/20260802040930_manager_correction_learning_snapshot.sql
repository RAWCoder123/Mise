-- Include downward manager-correction samples in the operational planning
-- snapshot so Edge signal refresh can learn bounded chronic manager-correction
-- bias alongside receiving, waste, and count-shrink histories.

create index if not exists inventory_movements_restaurant_manager_correction_created_at_idx
  on public.inventory_movements (restaurant_id, created_at desc)
  where reason = 'manager_correction';
  ...
  return jsonb_build_object(
    ...
    'wasteHistory', coalesce((
      ...
      from (
        select jsonb_build_object(
          'inventoryItemId', movement.inventory_item_id,
          'quantityRemoved', coalesce(
            (movement.metadata->>'quantity_removed_applied')::numeric,
            greatest(0, movement.quantity_before - movement.quantity_after)
          ),
        ...
        from public.inventory_movements movement
        where movement.restaurant_id = p_restaurant_id
          and movement.reason = 'waste'
          and movement.quantity_before > 0
          and (
            (
              movement.metadata ? 'quantity_removed_applied'
              and (movement.metadata->>'quantity_removed_applied') ~ '^-?[0-9]+(\\.[0-9]+)?$'
              and (movement.metadata->>'quantity_removed_applied')::numeric > 0
            )
            or movement.quantity_after < movement.quantity_before
          )
        order by movement.created_at desc, movement.id
        limit 500
      ) waste
    ), '[]'::jsonb),
    'countVarianceHistory', coalesce((
      select jsonb_agg(sample order by sample->>'createdAt' desc)
      from (
        select jsonb_build_object(
          'inventoryItemId', movement.inventory_item_id,
          ...
          'variance', coalesce(
            (movement.metadata->>'variance_from_system')::numeric,
            movement.quantity_after - movement.quantity_before
          ),
          'createdAt', movement.created_at,
          'sessionId', nullif(movement.metadata->>'session_id', '')
        ) as sample
        from public.inventory_movements movement
        where movement.restaurant_id = p_restaurant_id
          and movement.reason = 'manual_count'
          and movement.quantity_before > 0
          and coalesce(
            (movement.metadata->>'variance_from_system')::numeric,
            movement.quantity_after - movement.quantity_before
    ...
    'managerCorrectionHistory', coalesce((
      select jsonb_agg(sample order by sample->>'createdAt' desc)
      from (
        select jsonb_build_object(
          'inventoryItemId', movement.inventory_item_id,
          'quantityBefore', movement.quantity_before,
          'quantityAfter', movement.quantity_after,
          'variance', movement.quantity_after - movement.quantity_before,
          'createdAt', movement.created_at
        ) as sample
        from public.inventory_movements movement
        where movement.restaurant_id = p_restaurant_id
          and movement.reason = 'manager_correction'
          and movement.quantity_before > 0
          and movement.quantity_after < movement.quantity_before
        order by movement.created_at desc, movement.id
        limit 500
      ) manager_correction
    ), '[]'::jsonb)
...
comment on function private.fetch_operational_planning_snapshot(uuid, uuid) is
  'Service-owned planning snapshot for owner/admin/manager/staff, including receivingHistory, wasteHistory, countVarianceHistory, and managerCorrectionHistory for bounded operational learning. Edge enforces which mutations staff may run.';
