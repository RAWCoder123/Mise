...
begin
  ...
  alter table public.restaurant_memberships
    add constraint restaurant_memberships_role_check check (role in ('owner', 'admin', 'manager', 'staff'));
exception
...
begin
  ...
  alter table public.restaurant_memberships
    add constraint restaurant_memberships_status_check check (status in ('active', 'invited', 'disabled'));
exception
  when duplicate_object then null;
end $$;
  ...
  where conrelid = 'public.restaurant_memberships'::regclass
    ...
    and conkey = array[
      (
        select attnum
        from pg_attribute
        where attrelid = 'public.restaurant_memberships'::regclass
          and attname = 'user_id'
      )
    ];

  if constraint_name is not null then
    execute format('alter table public.restaurant_memberships drop constraint %I', constraint_name);
  end if;
end $$;

alter table public.restaurant_memberships
  add constraint restaurant_memberships_user_id_fkey
...
drop policy if exists "authenticated users can manage app users" on public.users;
drop policy if exists "authenticated users can manage restaurant memberships" on public.restaurant_memberships;
...
drop policy if exists "authenticated users can manage menu mappings" on public.menu_item_ingredients;
drop policy if exists "authenticated users can manage recommendations" on public.purchase_recommendations;
drop policy if exists "authenticated users can manage supplier orders" on public.supplier_orders;
