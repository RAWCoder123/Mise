...
| Forecast / outlook | `operationalSignals`, recalculation cycles, run ledger | PARTIAL | Open/mid/close cycles currently run the same recomputation; session dispatch now runs from Home and Today; unattended execution still needs an authorized machine-runner | Differentiate close reconciliation and add an authorized machine-runner before unattended operation |
