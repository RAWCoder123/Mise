...
| Supplier draft | approve RPC groups recommendations into one tenant/supplier draft, rebuilds idempotently, and dual-writes durable `supplier_order_lines`; receive uses those snapshots for ordered quantities | PARTIAL | Totals remain non-authoritative when costs are incomplete; discrepancy checklist UI remains separate (#182) | Keep totals absent unless every line has authoritative cost |
