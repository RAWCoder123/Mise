...
| Supplier draft | approve RPC groups recommendations into one tenant/supplier draft, rebuilds idempotently, and dual-writes durable `supplier_order_lines` | PARTIAL | Totals remain non-authoritative when costs are incomplete; receive still prefers live recommendations until a follow-up cutover | Keep totals absent unless every line has authoritative cost; point receive at durable lines after #182/#184 land |
