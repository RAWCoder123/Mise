...
| Supplier draft | approve RPC groups recommendations into one tenant/supplier draft, rebuilds idempotently, and dual-writes durable `supplier_order_lines`; send fingerprint and receive both bind those snapshots | PARTIAL | Totals remain non-authoritative when costs are incomplete; discrepancy checklist UI remains separate (#182) | Keep totals absent unless every line has authoritative cost |
...
| Delivery outcome | `record_supplier_delivery` → delivery rows + inventory receipt events + order completed; ordered quantities from durable `supplier_order_lines` | READY | Physical receipt was not proven against a real order; discrepancy checklist UI remains in #182 | Run one controlled receipt case; land discrepancy UI separately |
