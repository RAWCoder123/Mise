...
| Square locations | OAuth callback → `service_complete_square_oauth` → `pos_locations`; owner/admin authorize or pause via `set_pos_location_status` | READY | Sync uses `status='active'` only; reconnect preserves paused locations |
