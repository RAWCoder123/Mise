...
| Forecast / outlook | `operationalSignals`, recalculation cycles, run ledger, close reconciliation | PARTIAL | Close cycle now merges waste / count-variance / stock-carry insights and emits a reconciliation activity beat; open/mid still share planning recompute | Add an authorized machine-runner before unattended operation |
