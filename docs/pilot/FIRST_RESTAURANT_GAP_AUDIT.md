...
| Recommendation | `service_commit_operational_signals`, `purchase_recommendations`, learning history | PARTIAL | Readiness gates for create/approve live in open PRs; established decision patterns may boundedly advise quantity (MISE-004B) without changing authority | Land readiness/RPC gates; keep structured provenance from #130 |
...
| Future learning | approved quantities, recommendation decisions, outcomes, memories | PARTIAL | Purchase-decision patterns can now advise quantity (MISE-004B); purchase-loop receive/count variance measurement is still in open PRs #193/#194 | Land outcome measurement; keep authority gates closed |
