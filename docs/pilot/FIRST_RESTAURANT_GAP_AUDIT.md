...
| Catalog mapping | Square catalog → `menu_items` + draft `pos_catalog_item_mappings` | PARTIAL | New menu items are matched/created by normalized name and mapping stays draft until review; deleted/inactive Square catalog identities are soft-closed on full sync (`effective_to` + `expired`, orphan `menu_items.active = false`); no operator UI yet resolves remaining drafts | Keep draft mappings out of planning; add minimal mapping review UI; prove hosted catalog-deletion sync |
...
| --- | --- | --- |
| Square | OAuth/state/PKCE helpers, secret boundary, provider kill switches, normalized rows, role gates; full-sync catalog absence soft-close | Executed duplicate and overlapping database sync; invalid location; partial pagination; refresh failure after sales commit; hosted catalog-deletion proof |
