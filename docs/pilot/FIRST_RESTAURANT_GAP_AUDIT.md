...
| Recommendation | `service_commit_operational_signals`, `purchase_recommendations`, learning history | PARTIAL | Open PRs gate app/RPC approve+create and empty app-side generation; commit path empties pending system recommendations without canRecommend (stacked on #181). Evidence is still mainly a prose reason | Land readiness stack; persist structured provenance inputs |
