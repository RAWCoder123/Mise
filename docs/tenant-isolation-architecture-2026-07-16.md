...
| `service_save_restaurant_setup`, `service_record_setup_completion_audit` | Service-owned via Edge `save_setup`; manager+ actor membership; bounded atomic setup and fixed audit semantics; inventory quantity creates/deltas append `inventory_movements` (`manual_count` / `save_restaurant_setup`); legacy authenticated RPCs revoked |
