...
| iOS privacy manifest (required-reason APIs) | complete (code) | `expo.ios.privacyManifests` aggregates UserDefaults / FileTimestamp / DiskSpace reasons from AsyncStorage, Expo, and React Native `PrivacyInfo.xcprivacy` files; pinned by `qa:ios-prereq` + security unit test. Apple may still email additional ITMS-91053 reasons after first TestFlight upload. |
...
| Password reset | tested (code) | Login → Forgot password; `/reset-password` after recovery deep link; invalid/expired recovery callbacks surface localized login StatusNotice instead of silent bounce; signup/invite PKCE `?code=` callbacks exchange session without forcing recovery UI (recovery only for `type=recovery` or reset-password URLs); local `supabase/config.toml` allowlists `mise://reset-password` and Expo schemes; hosted Supabase Auth UI must mirror the same redirect allowlist |
...
| Tenant isolation / RLS | blocked | pgTAP public/private allowlists and count/storage probes updated for July/Aug tables; `security-backend` now also pins final authenticated table DML grants (SELECT-only) for Edge-owned tables; client membership revalidation now fails closed after tenant-authorization denials when live membership cannot be re-proven (periodic/foreground soft-fail retained); session membership list uses identity-free `list_my_restaurant_memberships()` excluding archived restaurants; restaurant-scoped hub/detail soft-refresh now fails closed on loadError so prior operational UI cannot stay ready after denial/refresh failure; locale/notification preference soft-loads keep loadError sticky and fail closed operational muting to defaults until prefs are re-verified; Inventory/Settings/Insights restaurant-scoped actions also require hub readiness (not stale membership alone); deep-link Inventory detail/count and Recipes mutations also require hub readiness before editability; Docker pgTAP + hosted staging must still re-run before pilot promotion |
