...
| Staff waste recording | tested (code) | Staff+ may record spoilage to the ledger; planning snapshot + signal commit allow staff for waste side effects; Edge audit allowlists staff-only action names; count/par edits stay manager+; Inventory list + staff detail elevate the waste path |
