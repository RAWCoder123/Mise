...
| Tenant isolation / RLS | blocked | pgTAP public/private allowlists and count/storage probes updated for July/Aug tables; `security-backend` now also pins final authenticated table DML grants (SELECT-only) for Edge-owned tables; client membership revalidation now fails closed after tenant-authorization denials when live membership cannot be re-proven (periodic/foreground soft-fail retained); Docker pgTAP + hosted staging must still re-run before pilot promotion |
