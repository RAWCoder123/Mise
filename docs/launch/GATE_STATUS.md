...
| 2. Inventory truth | In progress | Effective-dated mapping schema/domain rules, canonical conversions, append-only event ledger, replay-safe manager RPC, serialized AsyncStorage outbox/worker, hosted/demo submission adapter, bounded operator commands, count reconciliation | Inventory-item canonical-unit enforcement, hosted-staging adapter proof, receiving UI integration |
