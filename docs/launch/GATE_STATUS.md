...
| 2. Inventory truth | In progress | Effective-dated mapping schema/domain rules, canonical conversions, verified inventory-item unit authority, append-only event ledger, replay-safe manager RPC, serialized AsyncStorage outbox/worker, hosted/demo submission adapter, bounded operator commands, count reconciliation | Typed unit-verification adapter, hosted-staging proof, receiving/package-conversion UI integration |
