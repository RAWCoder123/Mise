...
export async function fetchOperatingBrief(
  ...
  const [data, orders, activityEvents, miseActions, findingDecisions, ledger, emailConnection] =
    await Promise.all([
  ...
  if (emailConnection && emailConnection.restaurant_id !== normalizedRestaurantId) {
    throw new Error("Operating brief failed email connection scope validation.");
  }
