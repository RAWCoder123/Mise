...
export function canActorCreateMemberInvite(
  ...
  inviteRole: AssignableTeamRole
): boolean {
  return assignableTeamRoles(actorRole).includes(inviteRole);
}
...
export function normalizeInviteEmail(email: string): string {
  return normalizeTeamMemberEmail(email) ?? email.trim().toLowerCase();
}

export function isValidInviteEmail(email: string): boolean {
  return normalizeTeamMemberEmail(email) !== null;
}
...
/** Shareable deep link for copying; in-app navigation still uses buildInviteClaimPath. */
export function buildInviteClaimDeepLink(token: string): string {
  return `mise:/${buildInviteClaimPath(token)}`;
}
