/**
 * Ranked find for More → Scan Item inventory list.
...
export type ScanItemInventorySearchFields = {
  id: string;
  item_name: string;
...
function scoreScanItemMatch(item: ScanItemInventorySearchFields, query: string): number | null {
...
/**
 * Rank inventory items for Scan Item search / browse.
 ...
 */
export function filterScanItemInventoryBySearch<T extends ScanItemInventorySearchFields>(
  items: readonly T[],
  query: string
): T[] {
  const unique: T[] = [];
  ...
  return unique
    .map((item, index) => {
      const score = scoreScanItemMatch(item, query);
