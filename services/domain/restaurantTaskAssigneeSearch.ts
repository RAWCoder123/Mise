/**
 * Ranked find for Create Task assignee picker.
 * Never invents members; only filters the caller-supplied assignable team list.
 */

export const RESTAURANT_TASK_ASSIGNEE_SEARCH_THRESHOLD = 8;

export type RestaurantTaskAssigneeSearchFields = {
  user_id: string;
  name?: string | null;
  email?: string | null;
  role?: string | null;
...
function primaryLabel(member: RestaurantTaskAssigneeSearchFields): string {
...
function scoreAssigneeMatch(
  member: RestaurantTaskAssigneeSearchFields,
  query: string
): number | null {
...
/**
 * Rank assignable restaurant members for the Create Task assignee picker.
 * Empty query preserves caller order (full deduped list). Non-empty query ranks
 * name/email/role matches without inventing rows.
 */
export function filterRestaurantTaskAssigneesBySearch<
  T extends RestaurantTaskAssigneeSearchFields
>(members: readonly T[], query: string): T[] {
  ...
  return unique
    .map((member, index) => {
      const score = scoreAssigneeMatch(member, query);
