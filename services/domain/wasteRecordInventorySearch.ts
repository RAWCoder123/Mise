/**
 * Ranked find for More → Waste → Record waste inventory picker.
...
export type WasteRecordInventorySearchFields = {
  id: string;
  item_name: string;
...
function scoreWasteRecordItemMatch(
  item: WasteRecordInventorySearchFields,
  query: string
): number | null {
...
/**
 * Rank inventory items for Waste record search / browse.
 ...
 */
export function filterWasteRecordInventoryBySearch<T extends WasteRecordInventorySearchFields>(
  items: readonly T[],
  query: string
): T[] {
  const unique: T[] = [];
  ...
  return unique
    .map((item, index) => {
      const score = scoreWasteRecordItemMatch(item, query);
