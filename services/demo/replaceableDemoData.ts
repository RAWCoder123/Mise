...
export function createInitialDemoState(
  ...
  const state: DemoState = {
    ...
    supplierItems: inventoryItems.map((item, index) => ({
      ...
      pack_quantity: item.unit === "lbs" ? 10 : null,
      verification_status: item.unit === "lbs" ? "verified" : "draft",
      verified_at: item.unit === "lbs" ? now : null,
      verified_by: item.unit === "lbs" ? DEMO_USER_ID : null,
...
function applyDefaultDemoDataset(state: DemoState, provider: PosProvider | null, createdAt: string, nowDate: Date) {
  ...
  state.supplierItems = state.inventoryItems.map((item, index) => ({
    ...
    pack_quantity: item.unit === "lbs" ? 10 : item.unit === "packs" ? 12 : null,
    verification_status: item.unit === "lbs" || item.unit === "packs" ? "verified" : "draft",
    verified_at: item.unit === "lbs" || item.unit === "packs" ? createdAt : null,
    verified_by: item.unit === "lbs" || item.unit === "packs" ? DEMO_USER_ID : null,
    estimated_unit_cost: item.estimated_unit_cost,
    preferred: true,
    created_at: createdAt,
